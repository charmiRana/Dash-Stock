...
def get_stock_price_fig(df):
    fig = pe.line(df, x="Date", y=["Close", "Open"], title = "Closing and Opening Price vs Date")
    return fig

def get_more(df):
    df['EWA_20'] = df['Close'].ewm(span=20, adjust=False).mean()
    fig = pe.scatter(df, x="Date", y= "EWA_20", title = "Exponential Moving Average vs Date")
    fig.update.traces(mode = 'lines+markers')
    return fig
        ...
        html.Div(
            [
                ...
                html.Div([
                    ...
                    html.Div([
                        doc.Input(id="dropdown_tickers", type="text"),
                ...
                html.Div([
                    doc.DatePickerRange(id='my-date-picker-range',
...
def stock_price(n, start_date, end_date, val):
    ...
    if val == None:
        ...
        if start_date != None:
            df = yfin.download(val, str(start_date), str(end_date))
        else:
            df = yfin.download(val)

    df.reset_index(inplace=True)
    fig = get_stock_price_fig(df)
    return [doc.Graph(figure=fig)]
...
def indicators(n, start_date, end_date, val):
    ...
    if start_date == None:
        df_more = yfin.download(val)
    else:
        df_more = yfin.download(val, str(start_date), str(end_date))
